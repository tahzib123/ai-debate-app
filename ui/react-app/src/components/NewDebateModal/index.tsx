...
export function NewDebateModal({ isOpen, onClose }: NewDebateModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    createPostMutation.mutate({
      ...
      created_by: 2, // Assuming user ID 2 is the current user (Rick)
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      {/* Modal */}
      <div className="relative bg-slate-800 rounded-xl border border-slate-700/50 p-6 w-full max-w-md mx-4 shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {createPostMutation.error && (
            <div className="p-3 bg-red-500/20 border border-red-500/50 rounded-lg text-red-400 text-sm">
              Failed to create debate post. Please try again.
            </div>
          )}
